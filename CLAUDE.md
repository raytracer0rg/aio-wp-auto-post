...
記事を生成しながら、以下の判断基準に当てはまる箇所を見つけたら、その直後に該当する EDIT HINT コメントを挿入する。挿入箇所と内容は、文章の流れと文脈に応じて毎回判断すること。形式の決まった場所への機械的な挿入は禁止する。
...
- 抽象的な説明が続いた箇所 → 具体例や事例の追加を促す
  ```
  <!-- wp:html -->
  <!-- ✏️ EDIT HINT: ここに自分が実際に経験した具体例を1つ加えると、抽象論が一気にリアルになります -->
  <!-- /wp:html -->
  ```
...
- データや調査結果を引用した箇所 → 自分のサイト・ツールでの検証データの追加を促す
  ```
  <!-- wp:html -->
  <!-- ✏️ EDIT HINT: 自分のサイトで同じ指標を測ってスクリーンショットを貼ると、一次情報としての価値が出ます -->
  <!-- /wp:html -->
  ```
...
- 一般論や定説を述べた箇所 → 筆者自身の見解・反対意見の追加を促す
  ```
  <!-- wp:html -->
  <!-- ✏️ EDIT HINT: ここに「自分はこう思う」という見解を1〜2文加えると、AIっぽさが消えて筆者の人格が出ます -->
  <!-- /wp:html -->
  ```
...
- 手順や方法を説明した箇所 → 実際にやってみた結果や失敗談の追加を促す
  ```
  <!-- wp:html -->
  <!-- ✏️ EDIT HINT: 実際に試した時のスクショや、ハマったポイントを加えると、読者の信頼度が上がります -->
  <!-- /wp:html -->
  ```
...
- ツールやサービスを紹介した箇所 → 自分の使用感や比較の追加を促す
  ```
  <!-- wp:html -->
  <!-- ✏️ EDIT HINT: 自分が使った時の率直な感想（良い点・物足りない点）を加えると、レビュー価値が高まります -->
  <!-- /wp:html -->
  ```
...
- 論争のあるテーマや異論がありそうな箇所 → 反対意見への配慮の追加を促す
  ```
  <!-- wp:html -->
  <!-- ✏️ EDIT HINT: ここに「ただし、こういう見方もある」という反論を1つ加えると、論考の厚みが出ます -->
  <!-- /wp:html -->
  ```

**挿入ルール：**

- 各ヒントは必ず `<!-- wp:html -->` と `<!-- /wp:html -->` で囲む
- 1記事あたり3〜6箇所が目安。多すぎても少なすぎてもいけない
- 同じ種類のヒントを連続で使わない（具体例を促した直後にまた具体例を促さない）
- 上記の例文をそのまま使わず、その箇所の文脈に合わせて文言を調整すること（例：「ここに〇〇というツールを使った結果を貼ると…」のように具体名を入れる）
